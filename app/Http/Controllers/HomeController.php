...
use App\Models\{Transaction, payment, kamar, SimpanKamar, Expense};
    ...
    public function index()
    {
        ...
        if (Auth::check()) {
            if ($auth == 'Admin') {
                ...
                $pengeluaran = Expense::where('user_id', Auth::id())->sum('amount');

                $pengeluaranMonth = Expense::where('user_id', Auth::id())
                    ->whereMonth('updated_at', Carbon::now()->format('m'))
                    ->whereYear('updated_at', Carbon::now()->format('Y'))
                    ->sum('amount');

                $pengeluaranYear = Expense::where('user_id', Auth::id())
                    ->whereYear('updated_at', Carbon::now()->format('Y'))
                    ->sum('amount');

                $pengeluaranPrevYear = Expense::where('user_id', Auth::id())
                    ->whereYear('updated_at', date("Y", strtotime("-1 year")))
                    ->sum('amount');
                ...
                return view('pemilik.index', \compact('aktif', 'total', 'pendapatan', 'pendapatanMonth', 'pendapatanYear', 'pendapatanPrevYear', 'jenis_kamar', 'stok_kamar', 'sisa_kamar', 'favorite','pengeluaran', 'pengeluaranMonth', 'pengeluaranYear', 'pengeluaranPrevYear'));
